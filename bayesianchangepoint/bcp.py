...
def readout(p_bar, r, beliefs, mode='expectation',threshold=40):
    """
    ...
    if mode=='expectation':
        ...
        for i in range(len(p_bar)-1):
            if i <= threshold :
                ...
                r_hat.append(threshold)
        p_hat = np.array([p_bar[r_hat[i], i+1] for i in range(len(r_hat))])
    # TODO : implement elif mode=='hindsight':
    return p_hat, r_hat

def plot_inference(o, p_true, p_bar, r, beliefs, mode='expectation', threshold=40, fig=None, axs=None, fig_width=13, max_run_length=120):
    ...
    p_hat, r_hat = readout(p_bar, r, beliefs, mode=mode, threshold=threshold)
    from scipy.stats import beta
